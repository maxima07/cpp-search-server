...
int main() {
    ...
    try {
        ...
        // Checking the addition of a negative ID
        //search_server.AddDocument(-1, "пушистый кот пушистый хвост"s, DocumentStatus::ACTUAL, {7, 2, 7});
        
        // Checking the addition of a document with a similar ID
        //search_server.AddDocument(1, "пушистый пёс и модный ошейник"s, DocumentStatus::ACTUAL, {1, 2});
        //search_server.AddDocument(1, "большой пёс скво\x12рец"s, DocumentStatus::ACTUAL, {1, 3, 2});
        
        // Checking documents for invalid characters
        //search_server.AddDocument(3, "большой пёс скво\x12рец"s, DocumentStatus::ACTUAL, {1, 3, 2});

        search_server.AddDocument(1, "пушистый пёс и модный ошейник"s, DocumentStatus::ACTUAL, {1, 2});
        search_server.AddDocument(2, "большой пёс скворец"s, DocumentStatus::IRRELEVANT, {1, 3, 2});

        // Checking the request for invalid characters
        //vector<Document> documents = search_server.FindTopDocuments("пуши\x12стый"s);
        //vector<Document> documents = search_server.FindTopDocuments("пуши\x12стый"s, DocumentStatus::IRRELEVANT);
        //vector<Document> documents = search_server.FindTopDocuments("пушис\x12тый пёс"s, [](int document_id, DocumentStatus status, int rating) {return document_id % 2 == 0; });
        //tuple<vector<string>, DocumentStatus> documents = search_server.MatchDocument("пуши\x12стый"s, 1);

        //Checking for extra characters before or after a word
        //vector<Document> documents = search_server.FindTopDocuments("--пушистый"s);
        //vector<Document> documents = search_server.FindTopDocuments("пушистый - "s);
        //vector<Document> documents = search_server.FindTopDocuments("--пушистый"s, DocumentStatus::IRRELEVANT);
        //vector<Document> documents = search_server.FindTopDocuments("пушистый - "s, DocumentStatus::IRRELEVANT);
        //vector<Document> documents = search_server.FindTopDocuments("--пушистый"s, [](int document_id, DocumentStatus status, int rating) {return document_id % 2 == 0; });
        //vector<Document> documents = search_server.FindTopDocuments("пушистый - "s, [](int document_id, DocumentStatus status, int rating) {return document_id % 2 == 0; });
        //tuple<vector<string>, DocumentStatus> documents = search_server.MatchDocument("--пушистый"s, 1);
        //tuple<vector<string>, DocumentStatus> documents = search_server.MatchDocument("пушистый - "s, 1);
